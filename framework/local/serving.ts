...
async function emulateSubruntime(handler: Handler, event: Event, context: Context, request: FastifyRequest, reply: FastifyReply) {
    ...
    try {
        ...
        return handleError(
            reply,
            new Error(`function invocation failed: ${err}`)
        );
    }
